...
- **לוג-סשן ב-Google Doc (כל סשן):** בכל סשן — פתח **doc-לוג נפרד** בתיקיית-Drive `StudiBuilder — לוגי-סשן` (id `1_GZY5fWK4z-BQRXUkySmsOUOPDnccVNw`, ב-`ACCESS-MAP.md`) דרך **MCP Google Drive** (`create_file` → כתיבה; ה-client של הפרויקט read-only). **עדכון-חי** אחרי כל משימה + **סיכום-סוגר** בסוף (מה נדחף · מה דורש-מוטי · הצעד-הבא) + **בלוק פרומפט-המשך** להעתקה. כותרת: `🟢 StudiBuilder · סשן <תאריך> · <נושא>`.
