...
> **🔓 מגן — port-permitted (עודכן 2026-06-09 · ה-firewall בוטל):** ריפו-מגן (`github.com/Moti316/megen`) — **מותר לפורט את פרומפט-המאסטר** (לחיבור-תרחישי-וועדה / סימולציה) ל-StudiBuilder, **name-cleaned** (להסיר "מגן"/"שגיא"/Telegram/מזהי-בעלים · לשמור 4-עקרונות · Zero-Harm · common-pitfalls · 3-מצבי-תשובה). מוטי בעל-שני-הריפו → **אפס-licensing**. megen **מבודד** (clone לתיקייה-סמוכה · read-only · מועתק רק **תוכן-פרומפט** · לעולם לא מנוע-ה-Python). שימוש: NotebookLM מעגן חוק/תקנה → פרומפט-מגן מחבר (Gemini-API · offline). ⛔ **מבטל את "כלל-מגן (firewall) — השראה-בלבד".** ראה `docs/architecture/ADR-009-magen-integration.md` (תיקון 2026-06-09).
