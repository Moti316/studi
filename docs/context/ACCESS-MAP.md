...
| `DATABASE_URL`                                               | **טעינת-שיעור (שאלות/chunks)**    | 🔴 **שורש-בעיה מאומת** (Vercel Logs 2026-06-04): `getaddrinfo ENOTFOUND db.<ref>.supabase.co` — ה-URL ב-Vercel הוא **חיבור-ישיר IPv6-only**, Vercel-serverless לא פותר. **תיקון:** החלף ל-**Transaction Pooler** (`postgres.<ref>@aws-0-<region>.pooler.supabase.com:6543`) + Redeploy. הקוד `prepare:false`. |
