...
> מעודכן: 2026-06-09 (פיבוט ADR-016 · סימולציית-וועדה + בנק-NotebookLM · אפס-Gemini ליצירה).
...
| 5 Quiz Engine     | 🟡 ~3/5     | `MatchingPairs`+`MCQShort`+`MCQLong`+`LessonPlayer`+`/lesson/[id]` ✅ · admin-תיוג `/admin/questions` ✅. חסר: מיני-קורס-תרחישים = סימולציית-וועדה היברידית ([ADR-016](../architecture/ADR-016-committee-simulation.md) · PrebakedEngine→LiveEngine · מחליפה את ה-`ScenarioWalkthrough` הסטטי), `ExplanationCard`, `/lesson/practice`+`/exam`, APIs |
...
- ✅ `GEMINI_API_KEY` מוגדר; **אך Gemini חסום-מכסה (20/יום)** → חיבור-תרחישים+שאלות ואימות-סמנטי = **Workflow רב-סוכני של Claude · אפס-Gemini** (ראה ADR-016 · DECISIONS 2026-06-09). **אין חוסמים טכניים** — הצינור בנוי; M5 חסום-תכנונית בלבד (אישורי מוטי 1→3 + צמצום discovery).
...
4. **M5** — מיני-קורס-שו"ת = בנק-NotebookLM רב-סוגי (~500 · mcq/matching/open מקורפוס-החקיקה · status=מוסקנא · אפס-Gemini) המחליף את בנק-ה-qa הישן (~540); מחיקת-הישן רק אחרי import+smoke. ראה [ADR-015](../architecture/ADR-015-notebooklm-content-engine.md).
...
- **מיני-קורס-תרחישים** — סימולציית-וועדה אינטראקטיבית ([ADR-016](../architecture/ADR-016-committee-simulation.md) · 3 מפקחים · 4 שלבים · ציון 0-100 · hybrid `PrebakedEngine`→`LiveEngine`) המחליפה את ה-`ScenarioWalkthrough` הסטטי. ✅ נבנו `src/features/simulation/{types,engine}.ts` (PrebakedEngine · 6 טסטים) + `SimulationPlayer.tsx`; נותר: צינור-חיבור (Claude+פרומפט-מגן+NotebookLM · אפס-Gemini) + שילוב `/lesson/scenarios` + החלפת 14 walkthroughs. + `ExplanationCard` + routes `/lesson/practice`+`/exam` + APIs.
