...
>
> 🔓 **עודכן 2026-06-09:** ה-firewall של מגן (השראה-בלבד · אסור-להעתיק · HYBRID) **בוטל** — port-מותר. אזכורי "מגן=השראה-בלבד" בפרומפט-ההיסטורי שלהלן (§פרומפט-המשך, סשן-C שהושלם) **מבוטלים** — ראה [`../architecture/ADR-009-magen-integration.md`](../architecture/ADR-009-magen-integration.md). (הערה: ה-firewall של **סקיל `agent-os`** — החרגת תוכן-מגן/דומיין מהערכה-הגנרית — נשאר בתוקף; זה גבול-genericness נפרד, לא כלל-ההשראה.)
