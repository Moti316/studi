...
- **🟢 הרשאה-עומדת (מוטי 2026-06-09):** Claude Code **תמיד מורשה** להריץ אוטונומית את סקריפטי-הגשר/ההפקה (`notebooklm login`/`list` · `scenarios`/`questions:generate`/`import` · `scenarios:author` · Gemini-offline) **ולבצע בדיקה-עצמית/verify** — בלי אישור פר-הרצה. login-אינטראקטיבי = הצעד-היחיד שדורש את מוטי (אם session פג: הרץ → יקפיץ חלון → המשך). ראה `content-gen-full-automation`.
