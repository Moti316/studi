...
- **🟢 מנוע-תוכן NotebookLM (Stage 1 · ADR-015 · 2026-06-08)** — צד-StudiBuilder נבנה ונבדק end-to-end (636 טסטים · dry-run מול קורפוס-אמיתי): שערי-G1–G5 (`verify-grounding.ts` · **G4 מחמיר = עיגון-מילולי+סעיף-חובה**) · importer (`src/lib/notebooklm/*` + `scripts/import-scenarios.ts`) · מיגרציה 0003 · גשר-Python (`tools/nblm-bridge/` · git-ignored). **בקרה-עצמאית** (oversight) → 2 קריטיים תוקנו (C1/C3) · BUGS.md#notebooklm-engine. 🚩 **דורש-מוטי:** apply-0003 (`tsx scripts/apply-migration-0003.ts`) + bootstrap-גשר (Python+`notebooklm login`) → ואז הרחבת-20-תרחישים. ⚠️ Python לא-מותקן במכונה.
